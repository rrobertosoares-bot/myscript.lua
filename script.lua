...
local function saveConfig()
	...
	if writefile then pcall(function() writefile("StilxzHubConfig.json",HttpService:JSONEncode(cfg)) end) end
end
...
local function setupChar(char)
	...
	if head then
		local old=head:FindFirstChild("StilxzSpeedBB");if old then old:Destroy() end
		local bb=Instance.new("BillboardGui",head)
		bb.Name="StilxzSpeedBB";bb.Size=UDim2.new(0,140,0,25)
...
-- ===================== GUI CLEANUP =====================
for _,n in pairs({"StilxzHubGUI"}) do
...
gui.Name="StilxzHubGUI";gui.ResetOnSpawn=false;gui.DisplayOrder=10;gui.IgnoreGuiInset=true
...
titleLbl.BackgroundTransparency=1;titleLbl.Text="STILXZ HUB"
...
mini.Name="StilxzMini";mini.Size=UDim2.new(0,120,0,26);mini.Position=UDim2.new(0,40,0,40)
mini.BackgroundColor3=SIDEBAR;mini.BorderSizePixel=0
mini.Text="STILXZ HUB";mini.TextColor3=ACCENT
...
local function loadConfig()
	if not(isfile and isfile("StilxzHubConfig.json")) then return end
	local ok,cfg=pcall(function() return HttpService:JSONDecode(readfile("StilxzHubConfig.json")) end)
